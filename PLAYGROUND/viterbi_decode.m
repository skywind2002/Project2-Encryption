%% 维特比译码函数，对 data 进行卷积码译码。
% data - 比特流或符号流，行向量。
% (n, k, m) - 卷积码参数
% A - 卷积码参数矩阵，大小为 n*k*m
% soft - 是否使用软维特比译码。
% mapping - 星座图。软维特比译码时使用。
function decode = viterbi_decode(data, n, k, m, A, soft, mapping)
    if(soft == 0) % Hard Viterbi
        distance = @(x, y)(sum(x ~= y, 2));
        data = [data, zeros(1, mod(-length(data), n))]; % 补零变成 n 的倍数
        data = reshape(data, n, []); % 化为若干列，每一列将译出 k bit（共依赖于 m 列）
    else % Soft Viterbi
        distance = @(x, y)(sum(abs(x - mapping(y + 1)).^2, 2));
    end
    n_decode = size(data, 2) * k; % 最终输出的码流长度
    p = 2; % 有限域为 GF(2)
    n_state = p^(m - 1); % 米利机状态数，p 种情形在输入中，p^{m-1} 种情形在状态中
    state = dec2base(0:n_state - 1, p, m-1) - '0'; % n_state 行 (m-1) 列矩阵，每一行对应一个长度为 (m-1) 的状态
    total_dis = [0, Inf + zeros(1, n_state - 1)]'; % 总距离。因为从状态为 0 开始，所以取第一个为 0、其余的为 Inf。
    route = zeros(n_state, size(data, 2)); % 存储最优路径，每一行都是一条走到相应 state 的最优路径。

    for i = 1:size(data, 2)
        input = data(:, i).'; % 当前处理的输入
        ...
        output = conv_encoding(full_state, n, k, m, A, 0, 0, p);
